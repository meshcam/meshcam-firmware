...
#ifdef ARDUINO
	// PATCHED for poe-bridge (LilyGo T-Internet-POE, LAN8720 ethernet):
	// upstream hardcodes WiFi.begin("wifi_ssid","wifi_password") and BLOCKS forever on
	// WL_CONNECTED — useless for a wired board. The sketch (main.cpp) brings the network
	// up via ETH.begin() BEFORE start() is called. WiFiUDP on ESP32 is a plain lwIP
	// socket, so udp.begin()/beginPacket() route over whichever interface is up (here:
	// ethernet). So we just open the socket; no WiFi.  -- see prototype README.
	Serial.println("[udp] start() over ethernet (network brought up by sketch)");
