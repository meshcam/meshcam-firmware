...
APP_NAME = "trailcam_gatea2"
ASPECT   = "resource"

# ----------------------------------------------------------------------------- server
def res_started(resource):
    RNS.log(f"[gateA2] incoming resource, size ~{resource.size} bytes")

def res_concluded(resource):
    if resource.status == RNS.Resource.COMPLETE:
        data = resource.data.read()
        RNS.log(f"[gateA2] RESOURCE COMPLETE: {len(data)} bytes  "
                f"sha256={hashlib.sha256(data).hexdigest()[:16]}")
    else:
        RNS.log(f"[gateA2] RESOURCE FAILED status={resource.status}", RNS.LOG_ERROR)

def link_established(link):
    RNS.log("[gateA2] leaf/board linked")
...
def server(configpath):
    ...
    RNS.log("[gateA2] SERVER destination hash: " + RNS.prettyhexrep(destination.hash))
    RNS.log("[gateA2] ^ paste the 32 hex chars into the board's a2_config.h, reflash")
    while True:                       # announce so the board can learn a path
        destination.announce()
        RNS.log("[gateA2] announced")
        time.sleep(15)

# ----------------------------------------------------------------------------- client
def send_concluded(resource):
    ok = resource.status == RNS.Resource.COMPLETE
    RNS.log(f"[gateA2] send {'COMPLETE' if ok else 'FAILED'}",
...
def client(dest_hex, configpath):
    ...
    if not RNS.Transport.has_path(dest_hash):
        ...
        RNS.log("[gateA2] requesting path...")
    ...
    def on_up(lnk):
        RNS.log("[gateA2] link up, sending 50 KB Resource")
